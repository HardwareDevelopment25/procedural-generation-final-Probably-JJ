...
public class MazeSpawner2D3D : MonoBehaviour
{
    ...
    [Header("Generation Mode: Changes between 2D and 3D Generation")]
    [SerializeField] private bool generate2D = false;
    ...
    private void Start()
    {
        ...
        if(generate2D)
            GetComponent<MeshRenderer>().material.mainTexture = ProcGenTools.DrawMaze2D(texture, maze);
        else
            DrawMaze3D();
    }

    public void GenImageOfNewMaze()
    {
        //incase 3D gen was on, stops thousands of mazes being created
        Start();
        generate2D = true;
    }
    ...
    private void DrawMaze3D() //loops through all maze x and y checking for true and placing floor otherwise if false places wall
    {
        ...
        for (int x = 0; x < size; x++)
        {
            for (int y = 0; y < size; y++)
            {
                if (maze[x, y])
                {
                    GameObject.Instantiate(floor, new Vector3(x, 0, y), Quaternion.identity, mazeParent.transform);
                }
                else
                {
                    GameObject.Instantiate(wall, new Vector3(x, 0, y), Quaternion.identity, mazeParent.transform);
                }
            }
        }
    }
